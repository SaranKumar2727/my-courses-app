...
function App() {
  ...
  const handleEditCourse = async (updatedCourse) => {
    try {
      const response = await axios.put(`http://localhost:5000/api/courses/${updatedCourse.courseid}`, updatedCourse);
  ...
  const handleEnrollCourse = async (courseId) => {
    ...
    try {
      const response = await axios.post(`http://localhost:5000/api/courses/enroll/${courseId}`,{userId},
  ...
  const handleUnenrollCourse = async (courseId) => {
    ...
    try {
      const response = await axios.post(`http://localhost:5000/api/courses/unenroll/${courseId}`, { userId });
      setCourses((prevCourses) =>
        prevCourses.map((course) =>
          course.courseid === courseId ? response.data.course : course
        )
